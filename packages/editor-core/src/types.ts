...
export function makeTrack(partial: Partial<Track> & Pick<Track, 'id' | 'kind' | 'name'>): Track {
  return { clips: [], locked: false, visible: true, muted: false, solo: false, ...partial };
}
...
export function makeSequence(id: string, name: string, aspect: AspectRatio = '16:9'): Sequence {
  ...
  return {
    ...
    markers: [],
    tracks: [
      makeTrack({ id: `${id}-v1`, kind: 'video', name: 'Video 1' }),
      makeTrack({ id: `${id}-v2`, kind: 'video', name: 'B-roll' }),
      makeTrack({ id: `${id}-t1`, kind: 'text', name: 'Text' }),
      makeTrack({ id: `${id}-c1`, kind: 'caption', name: 'Captions' }),
      makeTrack({ id: `${id}-a1`, kind: 'audio', name: 'Audio 1' }),
      makeTrack({ id: `${id}-a2`, kind: 'audio', name: 'Music' }),
    ],
  };
}
